...
export default function ShirtDesigner({
  ...
  useEffect(() => {
    setX((prev) =>
      clamp(prev, PRINT_AREA.x, PRINT_AREA.x + PRINT_AREA.width - avatarWidth)
    );
    setY((prev) =>
      clamp(prev, PRINT_AREA.y, PRINT_AREA.y + PRINT_AREA.height - avatarHeight)
    );
  ...
  const animateToCart = () => {
    ...
    const startX = stageBox.left + stageBox.width / 2 - 60;
    const startY = stageBox.top + stageBox.height / 2 - 60;
    const endX = cartBox.left + cartBox.width / 2 - 20;
    const endY = cartBox.top + cartBox.height / 2 - 20;
  ...
  const handleNicknameDragEnd = (e: any) => {
    const halfTextWidth = Math.max(70, label.length * (nicknameSize * 0.28));
    ...
    const nextY = clamp(
      e.target.y(),
      PRINT_AREA.y + PRINT_AREA.height - 25,
      PRINT_AREA.y + PRINT_AREA.height + 75
    );
  ...
  return (
    <>
      <div className="mx-auto max-w-[1520px] px-6 pt-4 pb-8" style={{ fontFamily: "'Nunito', sans-serif" }}>
        <div className="mb-8">
          <h1 className="text-4xl font-black tracking-tight" style={{ fontFamily: "'Fredoka One', sans-serif" }}>
            Конструктор футболки
          </h1>
          <p className="mt-2 text-base font-semibold text-zinc-500">
            Перемещай персонажа, меняй цвет и настраивай дизайн
          </p>
        </div>
          ...
          <div className="rounded-[28px] border-2 border-zinc-900 bg-white p-6 space-y-6 self-start shadow-[4px_4px_0px_#18181b]">
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setAvatarType('head')} className={avatarType === 'head' ? robloxBtnActive : robloxBtnInactive}>
                  Только голова
                </button>
                <button onClick={() => setAvatarType('full')} className={avatarType === 'full' ? robloxBtnActive : robloxBtnInactive}>
                  Весь аватар
                </button>
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <button onClick={() => setShirtColor('white')} className={`flex items-center justify-center gap-2 ${shirtColor === 'white' ? robloxBtnActive : robloxBtnInactive}`}>
                  <span className="h-4 w-4 rounded-full border-2 border-zinc-400 bg-white inline-block shrink-0" />
                  Белая
                </button>
                <button onClick={() => setShirtColor('black')} className={`flex items-center justify-center gap-2 ${shirtColor === 'black' ? robloxBtnActive : robloxBtnInactive}`}>
                  <span className="h-4 w-4 rounded-full bg-zinc-900 inline-block shrink-0" />
                  Чёрная
                </button>
              </div>
            </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {(['S', 'M', 'L', 'XL'] as const).map((size) => (
                  <button
                    ...
                    className={`rounded-xl border-2 border-zinc-900 py-3 text-sm font-black transition-all shadow-[2px_2px_0px_#18181b] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_#18181b] ${
                      shirtSize === size ? 'bg-yellow-400 text-zinc-900' : 'bg-white'
                    }`}
                  >
                    {size}
                  </button>
            ...
            <div className="rounded-2xl border-2 border-zinc-900 overflow-hidden shadow-[2px_2px_0px_#18181b]">
              <button
                type="button"
                onClick={() => setShowSizeTable((prev) => !prev)}
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {FONTS.map((f) => (
                  <button
                    key={f.id}
                    onClick={() => setNicknameFont(f.id as FontId)}
                    className={`flex flex-col items-center gap-1 rounded-xl border-2 border-zinc-900 px-2 py-2.5 transition-all shadow-[2px_2px_0px_#18181b] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_#18181b] ${
                      nicknameFont === f.id ? 'bg-yellow-400 text-zinc-900' : 'bg-white'
                    }`}
                  >
                    <span className="text-lg leading-none font-bold" style={{ fontFamily: f.id }}>
                      {f.preview}
                    </span>
              ...
              <Layer>
                ...
                {showNickname && label.trim() && (
                  <Text text={label} x={nameX} y={nameY}
                    ...
                    rotation={nicknameRotation} draggable onDragEnd={handleNicknameDragEnd} />
                )}
              </Layer>
            </Stage>
          </div>
