...
function BackgroundLayer({ bgId, x, y, width, height }: { bgId: BgId; x: number; y: number; width: number; height: number }) {
  ...
  if (bgId === 'city') {
    const buildings = [
      { bx: 0, bh: height*0.5, bw: width*0.15 }, { bx: width*0.12, bh: height*0.7, bw: width*0.12 },
      { bx: width*0.22, bh: height*0.4, bw: width*0.1 }, { bx: width*0.30, bh: height*0.6, bw: width*0.14 },
      { bx: width*0.42, bh: height*0.8, bw: width*0.13 }, { bx: width*0.53, bh: height*0.55, bw: width*0.11 },
      { bx: width*0.62, bh: height*0.65, bw: width*0.12 }, { bx: width*0.72, bh: height*0.45, bw: width*0.13 },
    ...
    return (
      <>
        <Rect x={x} y={y} width={width} height={height}
          ...
          fillLinearGradientColorStops={[0, bg.colors[0], 0.5, bg.colors[1], 1, bg.colors[2]]} cornerRadius={16} />
        {buildings.map((b, i) => <Rect key={i} x={x+b.bx} y={y+height-b.bh} width={b.bw} height={b.bh} fill="#050a18" opacity={0.9} />)}
      </>
    );
  }
  ...
  return (
    <Rect x={x} y={y} width={width} height={height}
      fillLinearGradientStartPoint={{ x: 0, y: 0 }} fillLinearGradientEndPoint={{ x: 0, y: height }}
      fillLinearGradientColorStops={bg.colors.length === 2 ? [0, bg.colors[0], 1, bg.colors[1]] : [0, bg.colors[0], 0.5, bg.colors[1], 1, bg.colors[2]]}
      cornerRadius={16} />
  );
}
...
export default function ShirtDesigner({ headshotUrl, fullAvatarUrl, username, isAdmin = false }: Props) {
  ...
  const STAGE_WIDTH = 1080; const STAGE_HEIGHT = 1040;
  const [x, setX] = useState(405); const [y, setY] = useState(210);
  const [scale, setScale] = useState(0.92);
  const [nameX, setNameX] = useState(540); const [nameY, setNameY] = useState(700);
  const [flyPreview, setFlyPreview] = useState<null | { image: string; startX: number; startY: number; endX: number; endY: number }>(null);
  ...
  const mockupSize = mockupImage ? fitContain(mockupImage.width, mockupImage.height, MOCKUP_BOX.width, MOCKUP_BOX.height) : { width: MOCKUP_BOX.width, height: MOCKUP_BOX.height };
  ...
  const PRINT_AREA = { x: mockupX + mockupSize.width * 0.255, y: mockupY + mockupSize.height * 0.24, width: mockupSize.width * 0.49, height: mockupSize.height * 0.5 };
  const BG_AREA = { x: PRINT_AREA.x - 8, y: PRINT_AREA.y - 8, width: PRINT_AREA.width + 16, height: PRINT_AREA.height + 16 };
  const textFill = shirtColor === 'black' ? '#ffffff' : '#111111';
  const avatarBase = useMemo(() => avatarType === 'head' ? { width: 300, height: 300 } : { width: 280, height: 360 }, [avatarType]);
  const avatarWidth = avatarBase.width * scale; const avatarHeight = avatarBase.height * scale;

  useEffect(() => {
    const centeredX = PRINT_AREA.x + (PRINT_AREA.width - avatarWidth) / 2;
    const centeredY = avatarType === 'head' ? PRINT_AREA.y + (PRINT_AREA.height - avatarHeight) / 2 - 15 : PRINT_AREA.y + (PRINT_AREA.height - avatarHeight) / 2 - 35;
  ...
  const exportMockup = async () => {
    ...
    const PRINT_PX = Math.round(PRINT_AREA.width * PRINT_DPI_SCALE); const PRINT_PY = Math.round(PRINT_AREA.height * PRINT_DPI_SCALE);
    printCanvas.width = PRINT_PX; printCanvas.height = PRINT_PY;
    const ctx = printCanvas.getContext('2d')!; ctx.clearRect(0, 0, PRINT_PX, PRINT_PY);
    const fullDataUrl = stage.toDataURL({ pixelRatio: PRINT_DPI_SCALE });
    await new Promise<void>((resolve) => { const img = new window.Image(); img.onload = () => { ctx.drawImage(img, PRINT_AREA.x * PRINT_DPI_SCALE, PRINT_AREA.y * PRINT_DPI_SCALE, PRINT_PX, PRINT_PY, 0, 0, PRINT_PX, PRINT_PY); resolve(); }; img.src = fullDataUrl; });
    const printPngUrl = printCanvas.toDataURL('image/png');
    const pngLink = document.createElement('a'); pngLink.download = `robloshkaf-print-${username||'user'}-${shirtSize}.png`; pngLink.href = printPngUrl; pngLink.click();
    ...
    const { jsPDF } = (window as any).jspdf; const doc = new jsPDF({ orientation: 'portrait', unit: 'mm', format: 'a4' });
    const BLACK = [24,24,27] as const; const YELLOW = [255,217,61] as const; const GRAY = [120,120,120] as const;
    doc.setFillColor(...BLACK); doc.rect(0,0,210,28,'F'); doc.setTextColor(255,255,255); doc.setFontSize(20); doc.setFont('helvetica','bold'); doc.text('РОБЛОШКАФ — МАКЕТ ДЛЯ ТИПОГРАФИИ',15,18);
    const previewUrl = stage.toDataURL({ pixelRatio: 1.5 }); doc.addImage(previewUrl,'PNG',15,35,85,82); doc.setDrawColor(...BLACK); doc.setLineWidth(0.8); doc.rect(15,35,85,82);
    doc.setFontSize(8); doc.setTextColor(...GRAY); doc.setFont('helvetica','normal'); doc.text('Предварительный просмотр (не для печати)',15,122);
    doc.setFillColor(...YELLOW); doc.roundedRect(108,35,87,10,2,2,'F'); doc.setTextColor(...BLACK); doc.setFontSize(10); doc.setFont('helvetica','bold'); doc.text('ПАРАМЕТРЫ ЗАКАЗА',113,42);
    const params = [['Никнейм',username||'Demo User'],['Цвет',shirtColor==='white'?'Белая':'Чёрная'],['Размер',shirtSize],['Фон',BACKGROUNDS.find(b=>b.id===selectedBg)?.label||'Нет'],['Никнейм',showNickname&&label?label:'Нет']];
    let py=52; params.forEach(([k,v],i)=>{ if(i%2===0){doc.setFillColor(245,245,245);doc.rect(108,py-4,87,8,'F');} doc.setFontSize(8);doc.setFont('helvetica','bold');doc.setTextColor(...BLACK);doc.text(k+':',111,py);doc.setFont('helvetica','normal');doc.setTextColor(60,60,60);doc.text(String(v),155,py);py+=9; });
    doc.addImage(printPngUrl,'PNG',70,140,70,70); doc.setDrawColor(...BLACK); doc.setLineWidth(0.5); doc.rect(70,140,70,70);
    doc.setFillColor(...BLACK); doc.rect(0,282,210,15,'F'); doc.setTextColor(255,255,255); doc.setFontSize(8); doc.setFont('helvetica','normal'); doc.text(`Роблошкаф | robloshkaf.vercel.app | ${username||'user'} | ${new Date().toLocaleDateString('ru-RU')}`,15,291);
    doc.save(`robloshkaf-макет-${username||'user'}-${shirtSize}.pdf`);
  };

  const animateToCart = () => {
    const stage = stageRef.current; const cart = document.getElementById('cart-button');
    if (!stage || !cart) return;
    const stageBox = stage.container().getBoundingClientRect(); const cartBox = cart.getBoundingClientRect();
    setFlyPreview({ image: stage.toDataURL({ pixelRatio: 1 }), startX: stageBox.left+stageBox.width/2-60, startY: stageBox.top+stageBox.height/2-60, endX: cartBox.left+cartBox.width/2-20, endY: cartBox.top+cartBox.height/2-20 });
    window.setTimeout(() => setFlyPreview(null), 700);
  };

  const addToCart = () => {
    const previewDataUrl = stageRef.current?.toDataURL({ pixelRatio: 2 });
    const item = { id: crypto.randomUUID(), username: username||'Roblox User', shirtColor, size: shirtSize, nickname: showNickname?label:'', nicknameSize, nicknameRotation, nicknameFont, avatarUrl: currentAvatarUrl, avatarType, background: selectedBg, previewDataUrl, createdAt: new Date().toISOString() };
    const raw = localStorage.getItem('roblox-shirt-cart'); const current = raw ? JSON.parse(raw) : [];
    localStorage.setItem('roblox-shirt-cart', JSON.stringify([...current, item]));
    window.dispatchEvent(new Event('cart-updated')); animateToCart();
  };
  ...
  return (
    <>
        ...
        <div className="grid items-start gap-8 md:grid-cols-[320px_minmax(980px,1fr)]">
          <div className="rounded-[28px] border-2 border-zinc-900 bg-white p-6 space-y-6 self-start shadow-[4px_4px_0px_#18181b]">
            ...
            <div className="rounded-2xl border-2 border-zinc-900 overflow-hidden shadow-[2px_2px_0px_#18181b]">
              <button type="button" onClick={() => setShowSizeTable(prev=>!prev)} className="flex w-full items-center justify-between px-4 py-3.5 text-left font-black text-sm hover:bg-zinc-50 transition">
                <span>Размерная таблица</span><span className="text-zinc-500 text-xs">{showSizeTable?'▲':'▼'}</span>
              </button>
                  ...
                  <div className="space-y-2 border-t-2 border-zinc-900 px-4 py-4 text-sm">
                    {SIZE_TABLE.map((row) => (
                      <div key={row.size} className="flex justify-between gap-3 font-semibold">
                        <span className="font-black">{row.size}</span><span>Ширина: {row.width}</span><span>Длина: {row.length}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div>
              ...
              <input value={label} onChange={(e) => setLabel(e.target.value)} className="w-full rounded-xl border-2 border-zinc-900 px-4 py-2.5 text-sm font-semibold focus:outline-none focus:ring-2 focus:ring-yellow-400 transition shadow-[2px_2px_0px_#18181b]" placeholder="Введите ник"/>
            </div>
            ...
            <div className="space-y-3">
              ...
              <label className="flex items-center gap-2.5 text-sm font-semibold cursor-pointer">
                <input type="checkbox" checked={showNickname} onChange={(e) => setShowNickname(e.target.checked)} className="h-4 w-4 rounded accent-yellow-400"/>Показывать никнейм
              </label>
              <button onClick={() => setLabel('')} className="w-full rounded-xl border-2 border-zinc-900 px-4 py-2.5 text-sm font-black hover:bg-zinc-50 transition shadow-[2px_2px_0px_#18181b] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_#18181b]">Удалить никнейм</button>
            </div>
            ...
            <div>
              ...
              <input type="range" min="0.45" max="1.45" step="0.05" value={scale} onChange={(e) => setScale(Number(e.target.value))} className="w-full accent-yellow-400"/>
            </div>
            <div>
              <label className={robloxLabel}>Размер никнейма</label>
              <input type="range" min="18" max="54" step="1" value={nicknameSize} onChange={(e) => setNicknameSize(Number(e.target.value))} className="w-full accent-yellow-400"/>
            </div>

            <div>
              <label className={robloxLabel}>Поворот никнейма</label>
              <div className="grid grid-cols-3 gap-2">
                {[{label:'↺ -45°',action:()=>setNicknameRotation(p=>p-45)},{label:'Сброс',action:()=>setNicknameRotation(0)},{label:'↻ +45°',action:()=>setNicknameRotation(p=>p+45)}].map(({label:l,action})=>(
                  <button key={l} onClick={action} className="rounded-xl border-2 border-zinc-900 px-3 py-2.5 text-sm font-black hover:bg-zinc-50 transition shadow-[2px_2px_0px_#18181b] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[1px_1px_0px_#18181b]">{l}</button>
                ))}
              </div>
            </div>

            <div className="grid gap-3 pt-2 border-t-2 border-zinc-200">
              <button onClick={addToCart} className="w-full rounded-2xl border-2 border-zinc-900 bg-[#E02020] px-4 py-3.5 font-black text-white shadow-[4px_4px_0px_#18181b] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0px_#18181b] transition-all">
                🛒 Добавить в корзину
              </button>
              {isAdmin && (
                <button onClick={exportMockup} className="w-full rounded-2xl border-2 border-zinc-900 bg-yellow-400 px-4 py-3.5 font-black text-zinc-900 shadow-[4px_4px_0px_#18181b] hover:translate-x-[1px] hover:translate-y-[1px] hover:shadow-[2px_2px_0px_#18181b] transition-all">
          ...
          <div className="rounded-[32px] bg-white p-4 overflow-auto min-h-[1040px] border-2 border-zinc-900 shadow-[4px_4px_0px_#18181b]">
            <Stage width={STAGE_WIDTH} height={STAGE_HEIGHT} ref={stageRef}>
              <Layer>
                {mockupImage && <KonvaImage image={mockupImage} x={mockupX} y={mockupY} width={mockupSize.width} height={mockupSize.height}/>}
                {selectedBg !== 'none' && <BackgroundLayer bgId={selectedBg} x={BG_AREA.x} y={BG_AREA.y} width={BG_AREA.width} height={BG_AREA.height}/>}
                {avatarImage ? (
                  ...
                  <>
                    <Rect x={x} y={y} width={avatarWidth} height={avatarHeight} cornerRadius={24} fill={shirtColor==='black'?'#2f2f2f':'#ececec'} draggable onDragEnd={handleAvatarDragEnd}/>
                    <Text text={avatarType==='head'?'Голова Roblox':'Аватар Roblox'} x={x} y={y+avatarHeight/2-12} width={avatarWidth} align="center" fontSize={20} fontStyle="bold" fill={shirtColor==='black'?'#ffffff':'#111111'}/>
                  </>
                )}
                {showNickname && label.trim() && (
                  <Text text={label} x={nameX} y={nameY} offsetX={label.length*(nicknameSize*0.28)} offsetY={nicknameSize/2} fontSize={nicknameSize} fontStyle="bold" fill={textFill} fontFamily={nicknameFont} rotation={nicknameRotation} draggable onDragEnd={handleNicknameDragEnd}/>
                )}
              </Layer>
            </Stage>
          </div>
      ...
      {flyPreview && (
        <img src={flyPreview.image} alt="preview" className="pointer-events-none fixed z-[9999] rounded-xl border-2 border-zinc-900 bg-white shadow-[4px_4px_0px_#18181b]"
          style={{ left: flyPreview.startX, top: flyPreview.startY, width: 120, height: 120, objectFit: 'contain', animation: 'flyToCart 700ms ease-in-out forwards', '--fly-end-x': `${flyPreview.endX-flyPreview.startX}px`, '--fly-end-y': `${flyPreview.endY-flyPreview.startY}px` } as React.CSSProperties}/>
      )}
    </>
  );
}
